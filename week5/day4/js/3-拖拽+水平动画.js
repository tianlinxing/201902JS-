...
* */
/*
* 需求：在拖拽结束之后，盒子要做弹性势能动画。并且我们出手的速度越快，盒子运动的在水平方向上运动的次数越多。并且当运动到浏览器边界时要反弹回来。垂直方向上，如果运动到底部，也需要弹起来，然后还会落下，往复若干次最终停止在浏览器底部；
*
* */
/*
* 分析：抛物线运动是由两个方向的运动合成的：水平方向弹性势能往复运动；垂直方向往复运动；所以我们需要分两种动画实现，然后拖拽结束后同时开启这两个动画；即可实现抛物线的运动。
* 水平方向上：
*   1. 计算盒子在拖拽结束后的出手速度；
*   2. 在运动的过程当中，判断是否越界（不能超出浏览器的可视区域）；
*   3. 到达边界以后，让速度变成相反的方向（给速度乘以-1）
*   4. 动画是一个速度衰减的过程（给速度乘以一个小于1的数，如0.98）
*   5. 清除动画的时机：因为速度不断乘以一个小于1的数字会不断变小，小到给盒子带来的动画肉眼不可见的时候就要清除动画；
* */
/*
* 1. 计算初速度：在盒子被拖动的过程中，浏览器的mousemove事件触发时间是固定的。而速度是单位时间内走过的距离，我们现在把两次mousemove事件触发中间这段事件作为单位时间，求初速度的问题就转化成计算两次鼠标之间的距离。
*
*
*
* */

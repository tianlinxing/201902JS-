...
/*
* 垂直：
*   1. 下落的过程是一个加速过程，所以给top累加的值是一个越来越大的值；
*   2. 到底会反弹起来，所以需要处理越界判断，反弹就是给速度乘等于-1；
...
function dragEnd(e) {
  ...
  // 因为拖拽时，mouseup时只做一件事情，就是移除document的mousemove事件，这样做可以让盒子不再跟随鼠标；但是后期我们给盒子增加了动画，并且是在mouseup时开启动画，这时如果不清除document的mouseup事件，就会出现在任意地方点击鼠标然后抬起都会触发dragEnd方法进而开启动画，导致定时器累加；所以为了解决这个问题，我们需要在onmouseup事件触发时即拖拽结束后移除document的mouseup事件。
  ...
  this.dropTimer = setInterval(() => drop.call(this), 30); // 执行垂直方向的动画
...
function fly() {
  ...
  if (curL >= this.maxL) {
    curL = this.maxL;
    this.hSpeed *= -1; // 乘等于-1是让盒子到了右边界反弹；
  }
...
function drop() {
  ...
  if (!this.canBounce) this.canBounce = 0;

  // 处理垂直方向上的动画；垂直方向的动画就是不断修改盒子的top值。

  // 1. 累加速度，让速度变大
  this.dropSpeed += 5;
  this.dropSpeed *= 0.98; // 为了让速度衰减，我们乘以一个小于1的数字
